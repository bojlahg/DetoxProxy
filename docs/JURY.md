...
Вариации написания, которые тоже маскируются: строчными с телефона («иванов иван иванович, тел 89123456789»), с латинской «o» внутри фамилии («Иванoв»), латиницей с отчеством («Ivanov Ivan Ivanovich»), «Королев» при словаре с «Королёв», дата прописью («двенадцатого марта 1985 г.»), даты `мм.дд.гггг` и «год первым» (`1985.31.12`, `1985-12-31`), «серия 4509, номер 123456», ФИО в смешанном регистре («ИВАНОВА мария петровна»).
...
python3 tools/jury_check.py --bin target/release/detox-proxy
                                                             # все примеры инструкции и демо: ловушки, утечки, восстановление
```
