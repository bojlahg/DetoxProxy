...
`http://93.183.93.44:8080/demo` (или `localhost:8080/demo`): три блока — маскирование с подсветкой найденного, восстановление ответа LLM (с примерами «все падежи» и «искажённые метки»), путь через LLM с системным промптом. В каждом блоке — кнопки готовых примеров, текст можно вводить вручную. Вверху страницы — живая панель из `GET /stats`: RPS, TPS, p50/p99 и ошибки за последнюю минуту. Полные метрики Prometheus — `/metrics`.
