...
async def main():
    ...
    ap.add_argument("--only", default="", help="через запятую: запустить только эти тесты (имена как в выводе); no_crash выполняется всегда")
    ap.add_argument("--yaml", action="store_true",
                    help="сгенерировать check-config.yaml (схема SPEC §4) и передать прокси как --config <путь> вместо переменных окружения")
    args = ap.parse_args()
    ...
    argv = [a.strip('"') for a in shlex.split(args.cmd, posix=(os.name != "nt"))]
    if args.yaml:
        cfg_path = os.path.abspath("check-config.yaml")
        with open(cfg_path, "w", encoding="utf-8") as f:
            f.write(CHECK_CONFIG_YAML.format(port=port, a=port + 1, b=port + 2))
        argv += ["--config", cfg_path]
    if os.path.exists(argv[0]):
        argv[0] = os.path.abspath(argv[0])  # Windows не запускает относительный путь с подкаталогом
    ...
    try:
        ...
        if not up:
            ...
            all_tests = ["nonstream_passthrough", "stream_incremental", "client_cancel", "failover_before_first_byte",
                         "all_upstreams_down", "ttft_timeout_failover", "midstream_abort_no_retry", "metrics", "load_200_streams"]
            unknown = only - set(all_tests)
            if unknown:
                print(f"[FAIL] args: неизвестные тесты в --only: {sorted(unknown)}; доступны: {all_tests}", flush=True)
                suite.results.append({"name": "args", "ok": False, "detail": "неизвестные имена тестов"})
            _run = suite.run

            async def run_selected(name, coro, timeout=30):
                if not only or name in only:
                    await _run(name, coro, timeout)
            suite.run = run_selected
