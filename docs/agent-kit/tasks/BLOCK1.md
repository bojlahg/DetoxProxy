...
<task id="T28">
Привет. Новый набор ловушек-двойников `tests/data/tricky_negatives.jsonl` (243 строки, которые НЕ должны маскироваться, и 12 контрольных с настоящими ПДн, которые маскироваться должны). Сейчас 228 из 243 чистые, 5 дефектов (каждый встречается трижды — один и тот же текст в разном окружении):

1. «Накладная 9876 543210 оформлена утром» → address. «Накладная», «партия», «счёт-фактура», «артикул», «инвентарный номер», «тикет», «заказ» — маркеры документа, а не адреса; после них числовая последовательность не адрес и не паспорт.
2. «IMEI 356938035643809 выбит под крышкой» → card_number. У IMEI (15 цифр) контрольная сумма Луна тоже сходится. Правило: 15 цифр подряд — не карта (карта 16, реже 13/14/18/19); плюс маркер IMEI/штрихкод/EAN/инвентарный — запрет.
3. «Горячая линия 8 800 200-00-00 работает круглосуточно» → phone. Номера 8-800 (и +7 800) — сервисные, не персональные: не маскировать. Плюс маркер «горячая линия», «служба поддержки», «колл-центр» — запрет.
4. «Магазин «Мария Ра» открылся» → fio. Слово в кавычках-ёлочках после «магазин», «компания», «ООО», «кафе», «сеть», «поезд», «бренд» — название, не ФИО.
5. «Приёмная находится на ул. Ленина, д. 10» → address. Добавь «приёмная», «канцелярия», «касса», «пункт выдачи» в non_pii-маркеры адреса (там уже есть отделение, офис, филиал).

Контрольные строки в том же файле должны продолжать маскироваться: СНИЛС «123 - 456 - 789 01» (его находит T11), паспорт «45 03 123456», карта «4276 5500 1122 3347», ИНН, телефон «+7 (900) 123-45-67», код подразделения, CVV, дата рождения, ФИО «Морозов Андрей Петрович», адрес «г. Казань, ул. Баумана, д. 14, кв. 8», держатель «ИВАНОВ ИВАН».

Тесты — в tests/traps.rs, по одному на каждый из пяти пунктов плюс по одному контрольному позитиву рядом (чтобы правило не убило настоящие ПДн).

Приёмка: `! grep -rn 'map(|(i, _)| i + 1)' src/ && cargo clippy --all-targets -- -D warnings && cargo test && cargo build --release && python tools/check_process.py --bin target/release/detox-proxy.exe --config config.yaml && bash tools/manual_accept.sh && bash tools/eval_accept.sh` (пороги в tests/eval_floors.txt уже включают `tricky_negatives:clean>=210` — доведи до 225).
...
<task id="T28c">
Привет. Регрессия после T28: в `data/pii_types.yaml` у `address` в `non_pii_markers` попали маркеры документов — «накладная», «партия», «счёт-фактура», «артикул», «инвентарный номер», «тикет», «заказ». Теперь настоящий адрес клиента рядом с этими словами не маскируется:
...
Это прямая утечка: в чатах поддержки «заказ» и «адрес доставки» идут рядом постоянно. Такие строки теперь есть в `tests/data/tricky_negatives.jsonl` (контрольные `trk-control-12`…`19`), сейчас из 5 адресов находится 1.

Что сделать:
1. Убери из `non_pii_markers` адреса все семь маркеров документов. «Приёмная», «канцелярия», «касса», «пункт выдачи» оставь — это адреса организаций.
2. Ловушка «Накладная 9876 543210 оформлена утром» должна по-прежнему не маскироваться. Её держит проверка в группировке адреса («non-PII маркер внутри спана»), но она читает тот же `effective_non_pii_markers` — после п. 1 она перестанет срабатывать. Переведи её на `DOCUMENT_MARKERS` (константа уже есть в `src/detect/mod.rs`): если спан адреса **начинается** с маркера документа — это номер документа, не адрес. Именно «начинается», а не «содержит»: «ул. Заказная, д. 5» — адрес.
3. `has_document_marker_before` для паспорта не трогай — там всё правильно.

Тесты в `tests/traps.rs`: три строки выше маскируются как адрес; «Накладная 9876 543210 оформлена утром» — нет.

Пороги в `tests/eval_floors.txt` уже подняты (`tricky_negatives:address>=5`), их не трогай.

Приёмка: `bash logs/T28c.accept.sh`

Файлы: data/pii_types.yaml, src/detect/mod.rs, tests/traps.rs. Каталог logs/ не трогай.
</task>

<task id="T39">
Привет. Самая обычная фраза получает неверный тип:

    Паспорт 4509 123456, выдан ОУФМС России по г. Москве 15.06.2015.
    → <<ADDR_1>>, выдан <<PASSISS_1>> <<ISSDATE_1>>.

Значение скрыто, но как адрес, вместе со словом «Паспорт». На демо жюри увидит это первым же запросом.

Причина — в `ADDRESS_COMPONENT_RES` (src/detect/mod.rs) последний шаблон `[А-ЯЁ][а-яё]+\s+\d+`: «улица без маркера + дом», как «Тверская 15». Он ловит «Паспорт 4509», а «123456» подходит под шаблон индекса `\b[1-6]\d{5}\b`. Вместе — группа адреса. Та же беда ждёт «Карта 4276…», «Телефон 8…», «Код 770…», «Заказ 12…», «Серия 45…».

Что сделать:
1. Компонент «Слово Число» не принимается, если слово — контекстное слово любого **другого** типа. Не список в коде, а данные: собери один раз при создании `Detector` множество `context_words` всех типов реестра, кроме `address`, в нижнем регистре, плюс `DOCUMENT_MARKERS`; однословные элементы проверяй по слову целиком. Так «паспорт», «карта», «инн», «снилс», «телефон», «серия», «номер» и всё, что добавят в YAML потом, перестанут считаться улицами автоматически.
2. Там же: слово в начале предложения, за которым идёт число, а до этого в той же группе нет ни города, ни маркера улицы — не улица. Проверь, не сломается ли «Москва, Тверская 15, кв. 3»: там город есть, это адрес.
3. Если после п. 1–2 паспорт всё ещё проигрывает адресу при пересечении спанов — посмотри, как решается пересечение, и отдай приоритет типу с валидатором или с явным маркером своего типа прямо перед спаном.

Тесты в `tests/traps.rs` (значение → ожидаемый тип; адреса в этих строках быть не должно):
- «Паспорт 4509 123456, выдан ОУФМС России по г. Москве 15.06.2015.» → passport;
- «Место рождения г. Казань. Паспорт 4509 123456.» → birth_place и passport;
- «Гражданство РФ. Паспорт 4509 123456, выдан вчера.» → passport;
- «Карта 4276 5500 1122 3347, срок 12/28.» → card_number;
Контроль, что адрес не пострадал:
- «Москва, Тверская 15, кв. 3» → address;
- «Проживает: г. Казань, ул. Баумана, д. 14, кв. 8» → address.

В `tests/data/tricky_negatives.jsonl` добавлены контрольные строки с этими фразами и полное анкетное письмо со всеми 17 типами (`trk-control-20`…`25`). Пороги в `tests/eval_floors.txt` подняты, не трогай их.

Приёмка: `bash logs/T39.accept.sh`

Файлы: src/detect/mod.rs, tests/traps.rs. Каталог logs/ не трогай.
</task>

<task id="T40">
Привет. Регрессия после T28 на `overlap_conflicts`: полнота упала со 100% до 94,7%, пропали 5 карт:

    Оплата картой 427640003331517, полис приложен.
    Полис ОМС 220015550000436 — проверьте оплату картой.

Правило T28 «15 цифр подряд без маркера карты — не карта» правильное, но `has_card_marker` ищет маркеры из `context.card_markers` в `data/pii_types.yaml`, а там только `карта` и `card`. «Картой», «карту», «карте», «карты» подстрокой «карта» не являются — маркер не находится.

Что сделать: допиши в `card_markers` падежные формы — карты, карте, карту, картой, картою, картам, картами, картах, карточка, карточки, карточке, карточку, карточкой. Больше ничего не меняй: IMEI-ловушка («IMEI 356938035643809 выбит под крышкой») должна по-прежнему не маскироваться — там маркера карты нет.

Тест в `tests/traps.rs`: две строки выше находят `card_number`, строка с IMEI — нет.

Приёмка: `bash logs/T40.accept.sh`

Файлы: data/pii_types.yaml, tests/traps.rs. Каталог logs/ не трогай.
</task>

<task id="T15">
Привет. Три способа, которыми ПДн проходят мимо детектора, даже без злого умысла:

    Клиент Иванoв Иван Иванович        (в «Иванoв» латинская o)  → фамилия остаётся открытой
    Клиент Королев Сергей Павлович     (в словаре только «королёв») → хуже находится
    Ivanov Ivan Ivanovich заходил вчера (латиница без маркера)   → не находится

**1. Буквы-двойники.** В `Detector::detect` перед поиском нормализуй текст: внутри слова, где есть хотя бы одна кириллическая буква, латинские двойники заменяются кириллическими: a→а, e→е, o→о, p→р, c→с, x→х, y→у, k→к, m→м, t→т, h→н, b→в и заглавные A B E K M H O P C T X Y. Слова целиком на латинице не трогай («Apple iPhone 15 Pro» остаётся как есть).

Главное — смещения. Латинская буква занимает 1 байт, кириллическая — 2, поэтому нормализованный текст длиннее. Строй вместе с ним таблицу «байтовое смещение в нормализованном тексте → смещение в исходном» (на каждой границе символа, плюс конец текста), ищи сущности в нормализованном и переводи `start`/`end` обратно по таблице. Если в тексте нет ни одного смешанного слова — работай с исходным текстом как сейчас, без таблицы (это обычный случай, он не должен стать медленнее).

**2. «ё» и «е».** В `Dictionaries` при загрузке и при поиске (`contains`) приводи «ё» к «е» — тогда «Королев», «Федор», «Семен» найдутся по словарю с «ё» и наоборот. Текст при этом не меняется, это только сравнение.

**3. ФИО латиницей.** Добавь в тип `fio` в `data/pii_types.yaml` шаблон для трёх слов латиницей с заглавной, где третье похоже на отчество: оканчивается на `ovich`, `evich`, `ovna`, `evna`, `ichna`, `ich` (например `Ivanov Ivan Ivanovich`, `Petrova Anna Sergeevna`). Без отчества латиница по-прежнему ищется только с маркером — «Apple Store Moscow» не ФИО.

Тесты — новый `tests/normalize.rs`:
- «Клиент Иванoв Иван Иванович» (латинская o) → одна сущность fio, и срез исходного текста по её `start..end` равен «Иванoв Иван Иванович» с латинской o;
- «ИНН 500100732259, Клиент Петрoва Анна» → inn и fio, смещения inn не сдвинулись;
- «Клиент Королев Сергей Павлович» и «Клиент Королёв Сергей Павлович» → fio в обоих;
- «Ivanov Ivan Ivanovich заходил вчера» и «Petrova Anna Sergeevna» → fio;
- «Apple iPhone 15 Pro Max», «Сервис Online Banking» → ничего;
- round-trip через `mask`/`unmask` на строке с латинской o возвращает исходный текст байт в байт.

Приёмка: `bash logs/T15.accept.sh`

Файлы: src/detect/mod.rs, data/pii_types.yaml, tests/normalize.rs. Каталог logs/ не трогай.
</task>

<task id="T41">
Привет. Жюри проверяет, что «определение не зависит от регистра». ВЕРХНИЙ регистр у нас работает, а строчные — нет:

    иванов иван иванович, тел 89123456789   → ФИО остаётся открытым
    клиент петрова анна сергеевна          → тоже

В чатах так пишут постоянно — с телефона, без заглавных.

Что сделать (src/detect/mod.rs, поиск ФИО):
1. Три подряд слова строчными, где есть фамилия по словарю, имя по словарю и отчество по словарю (порядки «Ф И О» и «И О Ф»), — это ФИО, уверенность как у того же ФИО с заглавными. Отчество обязательно: без него в нижнем регистре слишком много совпадений с обычными словами («роман», «вера», «надежда», «любовь», «лев»).
2. Два слова строчными (фамилия + имя в любом порядке) — ФИО только при маркере ФИО прямо перед ними («клиент», «фио», «гр.», «заявитель» — те же `context_words` типа `fio`).
3. Одно слово строчными — никогда не ФИО.
4. Смешанный регистр («иванов Иван Иванович») — как сейчас работает для заглавных; проверь, что не сломалось.

Тесты в `tests/traps.rs`:
- «иванов иван иванович, тел 89123456789» → fio на «иванов иван иванович» и phone;
- «клиент петрова анна сергеевна» → fio;
- «клиент петров иван» → fio;
- «иван иванович сказал, что перезвонит» → ничего: имя и отчество без фамилии и без маркера — обращение, а не ФИО;
- «роман толстого мы читали в школе», «вера в победу и надежда на лучшее», «любовь к родине» → ничего.

В `tests/data/tricky_negatives.jsonl` есть контрольные строки со строчными ФИО и ловушки из обычных слов; пороги в `tests/eval_floors.txt` уже подняты, не трогай их.

Заодно, одна строка: в `tests/perf_guard.rs` абсолютный бюджет `t400 <= 15_000` мкс слишком жёсткий, когда на машине параллельно идёт вторая сборка (падает и на старой ревизии: 17–18 мс). Подними до `25_000`. Проверку линейности `t4000 <= t400 * 15` не трогай — она и ловит квадрат.

Приёмка: `bash logs/T41.accept.sh`

Файлы: src/detect/mod.rs, tests/traps.rs, tests/perf_guard.rs. Каталог logs/ не трогай.
</task>

<task id="T43">
Привет. Большие тексты обрабатываются линейно, но медленно, если в них много слов с заглавной буквы. `/process`, лучшее из 3, на машине разработчика:

    обычная проза        849 КБ   200 мс
    Слова С Заглавной    761 КБ   729 мс     ← на сервере это ~2.5 с
    латиница             439 КБ    23 мс

Жюри проверяет «крупные тексты до 100 000 токенов» — это 400–800 КБ. Цель: текст из слов с заглавной (761 КБ) ≤ 200 мс, обычная проза (849 КБ) ≤ 100 мс, без потери качества.

**1. Замер.** Добавь в `tests/perf_guard.rs` второй тест: «Встреча Отдела Продаж Прошла В Москве, Обсудили Новые Условия Кредита. » × 10 000 (≈1.2 МБ), `Detector::detect` + `mask`, лучшее из 3 в release; `assert!(t <= 300_000)` микросекунд. В debug — ранний выход, как в первом тесте.

**2. Найди, куда уходит время.** Временно оберни в `Instant` крупные шаги `detect` (поиск по регулярным выражениям каждого типа, кандидаты ФИО, компоненты адреса, группировка адреса, разрешение пересечений) и выведи через `eprintln!` на этом тексте. Потом убери. Что обычно оказывается дорогим:
- `cyrillic_words(text)` вызывается несколько раз за запрос и на каждое слово делает `to_lowercase()` в новую `String` — посчитай один раз за запрос и передавай срез;
- словарные проверки каждого слова с заглавной (`dicts.contains` после `to_lowercase`) — те же слова проверяются в нескольких местах; кешируй результат на запрос (`Vec` по индексу слова);
- проверки контекста на каждого кандидата (`context_window_before(...).to_lowercase()` + `contains` по списку маркеров) — окно 40–200 символов на каждый из сотен тысяч кандидатов; для слов, которые не прошли словарь, до контекста дело доходить не должно;
- `number_follows_street` сортирует/сканирует все компоненты на каждое число.
Правь самое дорогое, замеряй снова, пока не уложишься.

**3. Не ломать.** Все пороги качества, ручные кейсы и контракт должны остаться зелёными; первый тест `perf_guard` (плотный текст) тоже.

Приёмка: `bash logs/T43.accept.sh`

Файлы: src/detect/mod.rs, tests/perf_guard.rs. Каталог logs/ не трогай.
</task>

<task id="T42">
Привет. Три пункта из критерия жюри «Безопасность, логирование и метрики». Каждый маленький, делай по порядку.

**1. Ключ доступа для системы.** Сейчас любую систему можно выбрать заголовком `X-System-Id`. Добавь в `SystemConfig` (src/config/mod.rs) необязательное поле `api_key_env: Option<String>` — имя переменной окружения с ключом этой системы.
- Если поле задано **и** переменная окружения непуста — запрос к этой системе без заголовка `X-Api-Key` или с неверным ключом получает `401` с телом `{"error":"unauthorized","code":"unauthorized"}` (формат как у остальных ошибок через `error_response`). Сравнение — за постоянное время (длины сравни отдельно, потом xor по байтам), ключ не попадает ни в логи, ни в метрики, ни в тексты ошибок.
- Если поле не задано или переменная пуста — всё как сейчас. **Система `autotest` должна продолжать работать без ключа**: по ней идёт нагрузочная проверка организаторов.
- Значение переменной читается при загрузке конфига (и при `/admin/reload`), не на каждый запрос.
- В `config.yaml` у системы `strict` добавь `api_key_env: DETOX_KEY_STRICT`. У `autotest` и `chatbot` не добавляй.
- Метрика `pii_rejected_total` получает `reason="unauthorized"`.

**2. Шифрование хранилища соответствий.** Оригиналы значений лежат в памяти открытым текстом. Шифруй поле `original` у каждой записи при `insert` и расшифровывай при `get` — снаружи API `MappingStore` не меняется.
- Крейт `chacha20poly1305` (RustCrypto), ключ 32 байта случайный, генерируется один раз при старте процесса (`OsRng`), нигде не сохраняется и не логируется. Nonce 12 байт — случайный на каждую запись.
- Внутри стора храни свою структуру (например, `SealedMapping { type_id, masked, nonce, ciphertext }`), а `Mapping` с открытым `original` существует только на время запроса.
- Если расшифровка не удалась (не должно случаться) — запись считается отсутствующей, в лог уходит `warn` без содержимого.
- Флаг в `server`: `encrypt_mappings: bool`, по умолчанию `true`.

**3. Этапы в журнале запроса.** В строку журнала запроса (там, где сейчас `latency_ms`) добавь поля в микросекундах: `detect_us`, `mask_us` (или `unmask_us` для восстановления), `store_us`, `total_us`. Отдельных строк не добавляй — одна строка на запрос, как сейчас.

Тесты:
- `tests/http.rs`: система с `api_key_env`, переменная окружения выставлена в тесте → без ключа 401, с неверным 401, с верным 200; `autotest` без ключа 200; тело 401 не содержит ключа.
- `tests/store.rs`: после `insert` во внутреннем представлении нет исходной строки (проверь через отладочный вывод `format!("{:?}", ...)` или специальный `#[cfg(test)]`-метод, который отдаёт сырые байты); `get` возвращает исходные `Mapping`.
- Журнал: строка запроса содержит `detect_us` и `total_us`.

Документы не трогай — README и JURY обновлю сам.

Приёмка: `bash logs/T42.accept.sh`

Файлы: src/config/mod.rs, src/server/mod.rs, src/store/mod.rs, Cargo.toml, Cargo.lock, config.yaml, tests/http.rs, tests/store.rs. Каталог logs/ не трогай.
</task>

<task id="T48">
Привет. Заголовки и строки рассылок, где каждое слово с заглавной, дают ложные ФИО:

    Встреча Отдела Продаж Прошла В Москве, Обсудили Новые Условия Кредита.
    → fio «Встреча Отдела Продаж», fio «Обсудили Новые Условия» (уверенность 0.6)
    Новые Условия Кредита Для Малого Бизнеса
    → fio «Новые Условия Кредита», fio «Для Малого Бизнеса»

Та же фраза строчными — чистая. В `tests/data/tricky_negatives.jsonl` теперь 15 таких строк (группа `titlecase`), все сейчас с масками; контрольная строка «Встреча С Клиентом Сидоровым Петром Ивановичем Перенесена» — ФИО должно по-прежнему находиться.

Найди, почему три слова с заглавной без единого словарного совпадения (ни имени, ни фамилии, ни отчества — с учётом падежных окончаний) получают 0.6 без маркера ФИО, и сделай правило: **окно ФИО, в котором ни одно слово не подтверждено словарями имён/фамилий/отчеств, — не ФИО, если прямо перед ним нет маркера ФИО** (`context_words` типа `fio`: «клиент», «ФИО», «заявитель»…). Окна, где словарь подтверждает хотя бы имя или отчество, работают как сейчас.

Проверь, что не сломались: ФИО латиницей с отчеством (T15), строчные ФИО (T41), ФИО в верхнем регистре («ИВАНОВ ИВАН ИВАНОВИЧ»), держатель карты латиницей («PETR SIDOROV» после «держатель»), ловушки знаменитостей.

Тесты в `tests/traps.rs`: две строки выше — без fio; контрольная строка — fio на «Сидоровым Петром Ивановичем».

Пороги в `tests/eval_floors.txt` уже подняты (`tricky_negatives:clean>=273`), не трогай их.

Приёмка: `bash logs/T48.accept.sh`
