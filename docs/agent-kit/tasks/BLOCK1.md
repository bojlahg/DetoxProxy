...
<task id="T28">
Привет. Новый набор ловушек-двойников `tests/data/tricky_negatives.jsonl` (225 строк, которые НЕ должны маскироваться, и 12 контрольных с настоящими ПДн, которые маскироваться должны). Сейчас 210 из 225 чистые, 5 дефектов (каждый встречается трижды — один и тот же текст в разном окружении):
