...
Весь код продукта написан coding-агентом DeepSeek-V4-Flash-0731 через AlfaGen (OpenCode, настройки — `opencode.json`). Архитектура, ТЗ с приёмкой, ревью, документация и презентация сделаны с ассистентом Claude (Anthropic) в роли архитектора и ревьюера, решения и проверка — за командой; роли и процесс — `docs/PROCESS.md`, правила агента — `AGENTS.md`.
...
- `docs/PROCESS.md` — как делалось: роли, ТЗ с XML-разметкой, исполняемая приёмка, статистика запусков агента.
